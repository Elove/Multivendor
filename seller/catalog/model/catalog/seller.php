...
class ModelCatalogSeller extends Model {
	...
	public function getTotalProductss($data = array(),$seller_id) {
		$sql = "SELECT COUNT(DISTINCT product_id) AS total FROM " . DB_PREFIX . "sellers_products 
		WHERE seller_id = '".(int)$seller_id . "' AND quantity>0 AND status=1";		
	...
	public function getProducts($data = array(),$seller_id=0) {
		...
		$sql .= " AND sp.seller_id = '" . (int)$seller_id. "' AND sp.quantity>0 AND sp.status=1";
