...
class ModelPaymentGlobalpayRemote extends Model {
	public function getMethod($address, $total) {
		...
		if ($this->config->get('globalpay_remote_total') > 0 && $this->config->get('globalpay_remote_total') > $total) {
			$status = false;
		} elseif (!$this->config->get('globalpay_remote_geo_zone_id')) {
			$status = true;
	...
	public function capturePayment($account, $amount, $currency, $order_id, $order_ref, $card_number, $expire, $name, $type, $cvv, $issue, $eci_ref, $eci = '', $cavv = '', $xid = '') {
		...
		if ($response->result == '00') {
			...
			if ($this->config->get('globalpay_remote_auto_settle') == 1) {
				$this->addTransaction($globalpay_order_id, 'payment', $order_info);
				$this->model_checkout_order->addOrderHistory($order_id, $this->config->get('globalpay_remote_order_status_success_settled_id'), $message);
			} else {
				$this->addTransaction($globalpay_order_id, 'auth', 0);
				$this->model_checkout_order->addOrderHistory($order_id, $this->config->get('globalpay_remote_order_status_success_unsettled_id'), $message);
			}
		} elseif ($response->result == "101") {
			// Decline
			$this->addHistory($order_id, $this->config->get('globalpay_remote_order_status_decline_id'), $message);
		} elseif ($response->result == "102") {
			// Referal B
			$this->addHistory($order_id, $this->config->get('globalpay_remote_order_status_decline_pending_id'), $message);
		} elseif ($response->result == "103") {
			// Referal A
			$this->addHistory($order_id, $this->config->get('globalpay_remote_order_status_decline_stolen_id'), $message);
		} elseif ($response->result == "200") {
			// Error Connecting to Bank
			$this->addHistory($order_id, $this->config->get('globalpay_remote_order_status_decline_bank_id'), $message);
