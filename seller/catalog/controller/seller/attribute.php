...
class ControllerSellerAttribute extends Controller {
	...
	if (!$this->seller->isLogged()) {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			$this->model_seller_attribute->addAttribute($this->request->post);
	...
	protected function getList() {
		...
		$data['sort_name'] = $this->url->link('seller/attribute','&sort=ad.name' . $url, 'SSL');
		$data['sort_attribute_group'] = $this->url->link('seller/attribute','&sort=attribute_group' . $url, 'SSL');
		$data['sort_sort_order'] = $this->url->link('seller/attribute','&sort=a.sort_order' . $url, 'SSL');
		$url = '';
		if (isset($this->request->get['sort'])) {
			$url .= '&sort=' . $this->request->get['sort'];
		}
		...
		$pagination->url = $this->url->link('seller/attribute',$url . '&page={page}', 'SSL');
		...
		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/seller/attribute_list.tpl')) {
			$this->response->setOutput($this->load->view($this->config->get('config_template') . '/template/seller/attribute_list.tpl', $data));
		} else {
	...
	protected function getForm() {
		...
		if (!isset($this->request->get['attribute_id'])) {
			...
			$data['action'] = $this->url->link('seller/attribute/edit','&attribute_id=' . $this->request->get['attribute_id'] . $url, 'SSL');
		}
		$data['cancel'] = $this->url->link('seller/attribute',$url, 'SSL');
		if (isset($this->request->get['attribute_id']) && ($this->request->server['REQUEST_METHOD'] != 'POST')) {
			$attribute_info = $this->model_seller_attribute->getAttribute($this->request->get['attribute_id']);
		}
		...
		if (isset($this->request->post['attribute_description'])) {
			...
			$data['attribute_description'] = $this->model_seller_attribute->getAttributeDescriptions($this->request->get['attribute_id']);
		} else {
		...
		$this->load->model('seller/attribute_group');
		$data['attribute_groups'] = $this->model_seller_attribute_group->getAttributeGroups();
		...
		$data['footer'] = $this->load->controller('common/footer');
		$data['header'] = $this->load->controller('common/header');
		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/seller/attribute_form.tpl')) {
			$this->response->setOutput($this->load->view($this->config->get('config_template') . '/template/seller/attribute_form.tpl', $data));
		} else {
			$this->response->setOutput($this->load->view('default/template/seller/attribute_form.tpl', $data));
		}
	}
	...
	protected function validateDelete() {
		...
		foreach ($this->request->post['selected'] as $attribute_id) {
			$product_total = $this->model_seller_product->getTotalProductsByAttributeId($attribute_id);
			if ($product_total) {
