...
class ControllerSellerFileManager extends Controller {
	public function index() {
		...
		$data['adminurl'] = HTTPS_SERVER1.'admin/';
		$this->load->model('seller/seller');
		$foldername = $this->model_seller_seller->getfoldername($this->seller->getId());
		// Make sure we have the correct directory
		if (isset($this->request->get['directory'])) {
			$directory = rtrim(DIR_IMAGE . $foldername.'/' . str_replace(array('../', '..\\', '..'), '', $this->request->get['directory']), '/');
		} else {
			$directory = DIR_IMAGE . $foldername;
		}
		...
		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/seller/filemanager.tpl')) {
			$this->response->setOutput($this->load->view($this->config->get('config_template') . '/template/seller/filemanager.tpl', $data));
		} else {
			$this->response->setOutput($this->load->view('default/template/seller/filemanager.tpl', $data));
		}
	}
