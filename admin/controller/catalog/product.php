...
class ControllerCatalogProduct extends Controller {
	...
	public function copy() {
		...
		if (isset($this->request->post['selected']) && $this->validateCopy()) {
            ...
            /*START OVICKO MULTISELLER*/
				...
				$data['approve'] = $this->url->link('catalog/product/approve', 'user_token=' . $this->session->data['user_token'] . $url, 'SSL');
	...
	protected function getList() {
		...
		if (isset($this->request->get['filter_approve'])) {			
			$filter_approve = $this->request->get['filter_approve'];		
		}
		else
		{	
			$filter_approve = null;
		...
		foreach ($results as $result) {
            ...
            /*START OVICKO MULTISELLER*/
	           //'seller_name'  => $sellername,
	            ...
	            'approve'     => ($result['approve'] ? $this->language->get('approve_enabled') : $this->language->get('approve_disabled')),			/*END OVICKO MULTISELLER*/
		...
		$data['sort_seller'] = $this->url->link('catalog/product', 'user_token=' . $this->session->data['user_token'] . '&sort=p.seller' . $url, true);
		$data['sort_approve'] = $this->url->link('catalog/product', 'user_token=' . $this->session->data['user_token'] . '&sort=p.approve' . $url, 'SSL');
			/*END OVICKO MULTISELLER*/
		...
		if (isset($this->request->get['order'])) {
            ...
            /*START OVICKO MULTISELLER*/
				...
				if (isset($this->request->get['filter_approve'])) {			$url .= '&filter_approve=' . $this->request->get['filter_approve'];		}
				$data['filter_seller_name'] = $filter_seller_name;
				$data['filter_approve'] = $filter_approve;
			/*END OVICKO MULTISELLER*/
    	...
    	public function approve() {
    	    ...
    	    foreach ($this->request->post['selected'] as $product_id) {
    			...
    			if ($product_info && !$product_info['approve']) {
    			$this->model_catalog_product->approve($product_id);
    			$approved++;
    			}
    		} 			
		...
		if (isset($this->request->get['product_id']) && ($this->request->server['REQUEST_METHOD'] != 'POST')) {
            ...
            /*START OVICKO MULTISELLER*/
				...
				if (isset($this->request->post['approve'])) {
					...
					$data['approve'] = $product_info['approve'];
				} else {
					$data['approve'] = '';
				}
				$this->load->model('sale/seller');
				...
				$data['seller_id'] =  $this->model_catalog_product->getProductSeller($this->request->get['product_id']);
				} else {
				$data['seller_id'] =  '';
				}
				...
				$seller_id =  $this->model_catalog_product->getProductSeller($this->request->get['product_id']);
				if($seller_id > 0){
				$name = $this->model_catalog_product->getProductSellerName($seller_id);
				$data['seller'] = $name;
				}else{
				$data['seller'] = "";
				}
				...
				$data['tab_seller_links'] = $this->language->get('tab_seller_links');
				$data['button_seller'] = $this->language->get('button_seller');
				$data['entry_seller_name'] = $this->language->get('entry_seller_name');
					$this->load->model('sale/seller');
					$data['sellers'] =
					$this->model_sale_seller->getSellers();		
					if (isset($this->request->post['product_seller'])) {
					$data['product_sellers'] = $this->request->post['product_seller'];
					} elseif (isset($this->request->get['product_id'])) {
					$data['product_sellers'] = $this->model_catalog_product->getProductSellers($this->request->get['product_id']);
					} else {
					$data['product_sellers'] = array();
					}
			/*END OVICKO MULTISELLER*/
