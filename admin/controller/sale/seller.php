...
class ControllerSaleSeller extends Controller { 
	private $error = array();
  	...
  	public function insert() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
      	  	...
      	  	$this->model_sale_seller->addSeller($this->request->post);
  	...
  	public function update() {
    	...
    	if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			$this->model_sale_seller->editSeller($this->request->get['seller_id'], $this->request->post);
  	...
  	public function delete() {
    	...
    	if (isset($this->request->post['selected'])) {
			foreach ($this->request->post['selected'] as $seller_id) {
				$this->model_sale_seller->approve($seller_id);
			}
	...
	public function approve() {
		...
		if ($sellers && $this->validateApprove()) {
			$this->model_sale_seller->approve($this->request->get['seller_id']);
	...
	public function approvepayment() {
		...
		if ($sellers && $this->validateApprove()) {
			$this->model_sale_seller->approvepayment($this->request->get['seller_id']);
  	...
  	private function getList() {
		...
		$data['approve'] = $this->url->link('sale/seller/approve1', 'user_token=' . $this->session->data['user_token'] . $url, 'SSL');
		$data['insert'] = $this->url->link('sale/seller/insert', 'user_token=' . $this->session->data['user_token'] . $url, 'SSL');
 ...
 $data['entry_commission'] = $this->language->get('entry_commission');
	...
	public function login() {
		...
		if ($seller_info) {
			...
			if ($store_info) {
				$this->response->redirect($store_info['url'] . 'index.php?route=seller/login&user_token=' . $user_token);
			} else { 
				$this->response->redirect(HTTPS_CATALOG1 . 'index.php?route=seller/login&user_token=' . $user_token);
			}
		} else {
	...
	public function transaction() {
    	$this->language->load('sale/seller');
		
		$this->load->model('sale/seller');
		
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->user->hasPermission('modify', 'sale/seller')) { 
			$this->model_sale_seller->addTransaction($this->request->get['seller_id'], $this->request->post['description'], $this->request->post['amount']);
		...
		$results = $this->model_sale_seller->getTransactions($this->request->get['seller_id'], ($page - 1) * 10, 10);
		...
		$data['balance'] = $this->currency->format($this->model_sale_seller->getTransactionTotal($this->request->get['seller_id']), $this->config->get('config_currency'));
		
		$transaction_total = $this->model_sale_seller->getTotalTransactions($this->request->get['seller_id']);
		...
		$pagination->limit = 10; 
		$pagination->text = $this->language->get('text_pagination');
		$pagination->url = $this->url->link('sale/seller/transaction', 'user_token=' . $this->session->data['user_token'] . '&seller_id=' . $this->request->get['seller_id'] . '&page={page}', 'SSL');
			
		$data['pagination'] = $pagination->render();

		$this->template = 'sale/seller_transaction';		
	...
	public function reward() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->user->hasPermission('modify', 'sale/seller')) { 
			$this->model_sale_seller->addReward($this->request->get['seller_id'], $this->request->post['description'], $this->request->post['points']);
		...
		$results = $this->model_sale_seller->getRewards($this->request->get['seller_id'], ($page - 1) * 10, 10);
		...
		$data['balance'] = $this->model_sale_seller->getRewardTotal($this->request->get['seller_id']);
		
		$reward_total = $this->model_sale_seller->getTotalRewards($this->request->get['seller_id']);
		...
		$pagination->limit = 10; 
		$pagination->text = $this->language->get('text_pagination');
		$pagination->url = $this->url->link('sale/seller/reward', 'user_token=' . $this->session->data['user_token'] . '&seller_id=' . $this->request->get['seller_id'] . '&page={page}', 'SSL');
			
		$data['pagination'] = $pagination->render();

		$this->template = 'sale/seller_reward';		
	...
	public function addBlacklist() {
		...
		if (isset($this->request->post['ip'])) { 
			if (!$this->user->hasPermission('modify', 'sale/seller')) {
				...
				$this->model_sale_seller->addBlacklist($this->request->post['ip']);
	...
	public function removeBlacklist() {
		...
		if (isset($this->request->post['ip'])) { 
			if (!$this->user->hasPermission('modify', 'sale/seller')) {
				...
				$this->model_sale_seller->deleteBlacklist($this->request->post['ip']);
		...
		if (isset($this->request->get['filter_name']) || isset($this->request->get['filter_email'])) {
			...
			foreach ($results as $result) {
				$json[] = array(
					...
					'address'           => $this->model_sale_seller->getAddresses($result['seller_id'])
				);					
			}
		}
	...
	public function upload() {
		...
		if (!$json) {
			if (is_uploaded_file($this->request->files['file']['tmp_name']) && file_exists($this->request->files['file']['tmp_name'])) {
				...
				$foldername = $this->model_sale_seller->getfoldername($this->request->get['seller_id']);
	...
	public function address() {
		...
		if (!empty($this->request->get['address_id'])) {
			...
			$json = $this->model_sale_seller->getAddress($this->request->get['address_id']);
		}
