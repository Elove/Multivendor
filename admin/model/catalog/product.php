...
class ModelCatalogProduct extends Model {
	public function addProduct($data) {
		...
		if (isset($data['product_option'])) {
            ...
            /*START OVICKO MULTISELLER*/
				$this->db->query("INSERT INTO " . DB_PREFIX . "product_option SET product_id = '" . (int)$product_id . "', option_id = '" . (int)$product_option['option_id'] . "', required = '" . (int)$product_option['required'] . "', seller_id = '" . (int)$product_option['seller_id'] . "'");
			/*END OVICKO MULTISELLER*/
            ...
            /*START OVICKO MULTISELLER*/
			$this->db->query("INSERT INTO " . DB_PREFIX . "product_option SET product_id = '" . (int)$product_id . "', option_id = '" . (int)$product_option['option_id'] . "', value = '" . $this->db->escape($product_option['value']) . "', required = '" . (int)$product_option['required'] . "', seller_id = '" . (int)$product_option['seller_id']. "'");
			/*END OVICKO MULTISELLER*/
	...
	public function getSellers() {	
		$sellers_data = array();
		$query = $this->db->query("SELECT distinct(v.seller_id), v.username AS name  FROM " . DB_PREFIX . "sellers as v," . DB_PREFIX . "seller as p where p.seller_id = v.seller_id ORDER BY v.firstname");
		$sellers = $query->rows;
		if(count($sellers)>0){
			foreach($sellers as $seller){
				$sellers_data[$seller['seller_id']] = $seller;
			}
		}
		return $sellers_data;
	}
	public function getSellers1() {	
		$sellers_data = array();
		$query = $this->db->query("SELECT distinct(v.seller_id), v.username AS name  FROM " . DB_PREFIX . "sellers as v," . DB_PREFIX . "seller as p where p.seller_id = v.seller_id ORDER BY v.firstname");
		$sellers = $query->rows;
		// if(count($sellers)>0){
	...
	public function getProductSellers($product_id) {
		...
		return $query->rows;
				...
				public function getTotalProducts1($data = array()) {
        $approve=0;
