...
class ModelCheckoutOrder extends Model {
	public function addOrder($data) {
		...
		if (isset($data['products'])) {
			...
			if($seller_id>0){
				$get_seller_commissionid = $this->db->query("SELECT c.commission_id,commission 
				FROM `" . DB_PREFIX . "commission` as c,`" . DB_PREFIX . "sellers` as s 
				WHERE s.seller_id ='".$seller_id."' AND c.commission_id = s.commission_id");
				if($get_seller_commissionid->row['commission_id']){
					$comm_id = $get_seller_commissionid->row['commission_id'];
					$get_seller_commission = $this->db->query("SELECT max(commission_rate) as commission_rate FROM `" . DB_PREFIX . "commission_rates` as cr,`" . DB_PREFIX . "product_to_category` as pc WHERE cr.commission_id = '".(int)$comm_id."' AND pc.category_id=cr.category_id AND pc.product_id='".(int)$product['product_id']."' limit 1");
					if($get_seller_commission->row['commission_rate']>0){
						$get_seller_id->row['commission'] = $get_seller_commission->row['commission_rate'];
					}else{
						$get_seller_id->row['commission'] = $get_seller_commissionid->row['commission'];;
					}
				}
				else{
					$get_seller_id->row['commission'] = 0;
				}
				$shippingcharges = 0;
				$tprice = $product['total'];				
				$com = $tprice*((float)$get_seller_id->row['commission']/100);				
				$seller_total = $tprice - $com;
				$this->db->query("INSERT INTO " . DB_PREFIX . "order_product SET order_id = '" . (int)$order_id . "', 
				seller_id =  '" . $seller_id. "', commission =  '" . (float)$product['total']*((float)$get_seller_id->row['commission']/100) . "',
				seller_total = '" . (float)(($product['total']*(1-((float)$get_seller_id->row['commission'])/100))) . "',
				product_id = '" . (int)$product['product_id'] . "',
				name = '" . $this->db->escape($product['name']) . "', model = '" . $this->db->escape($product['model']) . "', 
				quantity = '" . (int)$product['quantity'] . "', price = '" . (float)$product['price'] . "',
				total = '" . (float)$product['total'] . "', tax = '" . (float)$product['tax'] . "', reward = '" . (int)$product['reward'] . "'");
				$order_product_id = $this->db->getLastId();
				$description = "Order Id# ".$order_id." ".$product['name'] .":".$this->config->get('config_currency').$product['total']." (".$get_seller_id->row['commission']."% Commission";
				$description .= ")"; 
				$amt = ($product['total']*(1-((float)$get_seller_id->row['commission'])/100));
				$this->db->query("INSERT INTO " . DB_PREFIX . "seller_transaction SET 
					seller_id = '" . (int)$seller_id. "', 
					order_id = '" . (int)$order_id . "',
                    order_product_id = '" . (int)$order_product_id . "',					
					description = '" . $this->db->escape($description) . "',
					sub_total = '" . (float)$tprice . "',
	...
	public function getOrderTotals($order_id) {
		...
		if ($order_info) {
			...
			WHERE product_id = '" . (int)$order_product['product_id'] . "' AND subtract = '1' AND
				seller_id = '" . (int)$order_product['seller_id'] . "'");
			$this->db->query("UPDATE " . DB_PREFIX . "sellers_products SET quantity = (quantity - " . (int)$order_product['quantity'] . ") 
			WHERE product_id = '" . (int)$order_product['product_id'] . "' AND seller_id = '" . (int)$order_product['seller_id'] . "'");
			$this->db->query("UPDATE " . DB_PREFIX . "order_product SET 
			product_status_id = '" . (int)$order_status_id . "' where order_id = '" . (int)$order_id . "' AND seller_id='" . (int)$order_product['seller_id']."'");
			/*END OVICKO MULTISELLER*/
